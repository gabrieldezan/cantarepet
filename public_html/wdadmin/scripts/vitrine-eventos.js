$(document).ready(function () {
    ...
    $(this).attr("title", "WD Admin - Eventos da Vitrine");
    ...
    $("#botao_pesquisa_vitrine_eventos").click(function () {
        Loading();
        carrega_vitrine_eventos();
    });

    $('#tabela_vitrine_eventos').DataTable();
...
function carrega_vitrine_eventos() {
  
    $.ajax({
        url: vsUrl + "wdadmin/controllers/RetornaVitrineEventos.php",
        type: "POST",
        dataType: "json",
        async: false,
        success: function (data) {
            if (data !== 0) {
                $("#tabela_vitrine_eventos tbody").html("");
                $('#tabela_vitrine_eventos').DataTable().destroy();
                var table = $('#tabela_vitrine_eventos').DataTable({
                    "language": {
                    ...
                    buttons: [
                        ...
                        },
                        {
                            ...
                            action: function () {
                                window.location.href = "vitrine-eventos/cadastro";
                            }
                        }
                    ],
                    fixedHeader: false,
                    ...
                    "columns": [
                        ...
                        {
                            "render": function (data, type, row) {
                                return "<a href=\"vitrine-eventos/cadastro/" + row.id_vitrine_evento + "\" class=\"btn btn-sm btn-outline-secondary\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Editar " + row.descricao + "\"><i class=\"fas fa-edit\"></i></a>";
                            }
                        }
                    ],
                    'columnDefs': [
